...
#===============================
# 脚本更新时间（Script update time）：2023.09.03 13:49
# 脚本作者：ZJHCOFI
# Edit by ZJHCOFI
# 博客（Blog）：http://zjhcofi.com
# Github：https://github.com/zjhcofi
# 开源协议（License）：BSD 3-Clause “New” or “Revised” License
# 后续更新或漏洞修补通告页面（Subsequent updates and bug fixes notification page）：https://github.com/zjhcofi
...
function path_way_check() {
	...
	if [ "$path_way" != "${path_way//[\' ]/}" ]; then 
		print_error "当前的绝对路径有空格，请将脚本移至无空格的绝对路径下\nThe current absolute path has spaces. Please move the script to an absolute path without spaces."
		exit 1
	fi
}
...
function is_root() {
	if [[ 0 == "$UID" ]]; then
		print_ok "当前用户是 root 用户，开始安装流程\nThe current user is root, starting the installation process."
	else
		print_error "当前用户不是 root 用户，请切换到 root 用户后重新执行脚本\nThe current user is not root. Please switch to root and execute the script again."
		exit 1
	fi
}
...
${Red}4.${Font} 升级后清理(不包含备份文件)(Cleanup after upgrade, but does not include backup files.)
${Red}5.${Font} 执行修改ssh配置文件后的升级步骤(Perform the upgrade steps after modifying the SSH profile.)
${Red}6.${Font} 回退至以前的版本(Fallback to previous version.)
...
function system_check() {
	...
	if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
		print_ok "当前系统为 Centos ${VERSION_ID} ${VERSION}\nThe current system is Centos ${VERSION_ID} ${VERSION}"
		INS="yum install -y"
		RMS="yum remove -y"
		${INS} curl
		judge "安装 curl\nInstall curl"
		...
		print_error "当前系统为 ${ID} ${VERSION_ID} 不在支持的系统列表内\nThe current system is ${ID} ${VERSION_ID} ,not in the list of supported systems."
		exit 1
	fi
}
...
function openssh_check() {
	...
	if [[ ${openssh_file_num} == "0" ]];then
		print_error "脚本目录下没有OpenSSH版本包\nThere is no OpenSSH package in the script directory."
		openssh_version=""
		openssh_download
	elif [[ ${openssh_file_num} != "1" ]];then 
		print_error "脚本目录下只能存在一个OpenSSH版本包\nOnly one OpenSSH package can exist in the script directory."
		openssh_version=""
		openssh_download
	else
		openssh_version=$(ls ${path_way} | grep openssh-*p*.tar.gz)
		print_ok "检测到脚本目录下的OpenSSH版本包为 ${Font}${Red}${openssh_version}${Blue}，继续执行安装程序\nDetected that the OpenSSH package in the script directory is ${Font}${Red}${openssh_version}${Blue},continue installation."
...
function openssl_check() {
	...
	openssl_file_num=$(ls ${path_way} | grep openssl-1.*.*.tar.gz | wc -l)

	if [[ ${openssl_file_num} == "0" ]];then
		print_error "脚本目录下没有OpenSSL版本包\nThere is no OpenSSL package in the script directory."
		openssl_version=""
		openssl_download
	elif [[ ${openssl_file_num} != "1" ]];then
		print_error "脚本目录下只能存在一个OpenSSL版本包\nOnly one OpenSSL package can exist in the script directory."
		openssl_version=""
		openssl_download
	else
		openssl_version=$(ls ${path_way} | grep openssl-1.*.*.tar.gz)
		print_ok "检测到脚本目录下的OpenSSL版本包为 ${Font}${Red}${openssl_version}${Blue}，继续执行安装程序\nDetected that the OpenSSL package in the script directory is ${Font}${Red}${openssl_version}${Blue},continue installation."
...
function openssh_download() {
	...
	if [[ ${openssh_version} == "" ]];then
		...
		if [[ ${openssh_version} == "" ]];then
			print_error "获取OpenSSH版本包列表失败\nFail to obtain the list of OpenSSH packages."
			echo -e "\n${INFO}${Blue}请自行前往 ${Font}${Yellow}https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/ ${Font}${Blue}下载您需要升级的OpenSSH版本包，放在脚本同目录下。(注意：只能是${Font}${Yellow}p1.tar.gz${Font}${Blue}或${Font}${Yellow}p2.tar.gz${Font}${Blue}结尾的版本包)\nPlease download the OpenSSH version you need to upgrade from the ${Font}${Yellow}https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/${Font}${Blue},place it in the same directory as the script.\nAttention: Only versions ending in ${Font}${Yellow}p1.tar.gz${Font}${Blue} or ${Font}${Yellow}p2.tar.gz${Font}${Blue} can be used.${Font}\n"
			exit 1
		else
			print_ok "获取OpenSSH版本包列表成功\nObtain the list of OpenSSH packages success."
		fi
		...
		[yY][eE][sS] | [yY])
			...
			judge_download_ssh "下载OpenSSH版本包\nDownload OpenSSH package."
			;;
		*)
			echo -e "\n${INFO}${Blue}请自行前往 ${Font}${Yellow}https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/ ${Font}${Blue}下载您需要升级的OpenSSH版本包，放在脚本同目录下。(注意：只能是${Font}${Yellow}p1.tar.gz${Font}${Blue}或${Font}${Yellow}p2.tar.gz${Font}${Blue}结尾的版本包)\nPlease download the OpenSSH version you need to upgrade from the ${Font}${Yellow}https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/${Font}${Blue},place it in the same directory as the script.\nAttention: Only versions ending in ${Font}${Yellow}p1.tar.gz${Font}${Blue} or ${Font}${Yellow}p2.tar.gz${Font}${Blue} can be used.${Font}\n"
			exit 1
...
function openssl_download() {
	...
	if [[ ${openssl_version} == "" ]];then
		...
		openssl_version=$(curl https://www.openssl.org/source/ | grep openssl-1.*.* | cut -d '"' -f 2 | cut -d '"' -f 1 | tail -n 1)
		
		if [[ ${openssl_version} == "" ]];then
			print_error "获取OpenSSL版本包列表失败\nFail to obtain the list of OpenSSL packages."
			echo -e "\n${INFO}${Blue}请自行前往 ${Font}${Yellow}https://www.openssl.org/source/ ${Font}${Blue}下载您需要升级的OpenSSL版本包，放在脚本同目录下。(注意：只能是${Font}${Yellow}openssl-1.x.xx${Font}${Blue}的版本包)\nPlease download the OpenSSL version you need to upgrade from the ${Font}${Yellow}https://www.openssl.org/source/${Font}${Blue},place it in the same directory as the script.\nAttention:It can only be a version package of ${Font}${Yellow}1.x.xx${Font}\n"
			exit 1
		else
			print_ok "获取OpenSSL版本包列表成功\nObtain the list of OpenSSL packages success."
		fi
		
		echo -e "\n${CHOOSE}${Blue}最新的OpenSSL版本包为 ${Font}${Red}${openssl_version}${Font}${Blue}，是否升级到此版本？(y/n)\nThe latest OpenSSL version is ${Font}${Red}${openssl_version}${Font}${Blue},do you want to upgrade to this version? (y/n)：${Font}"
		read -r download_openssl
		case ${download_openssl} in
		[yY][eE][sS] | [yY])
			wget https://www.openssl.org/source/${openssl_version} --no-check-certificate
			judge_download_ssl "下载OpenSSL版本包\nDownload OpenSSL package."
			;;
		*)
			echo -e "\n${INFO}${Blue}请自行前往 ${Font}${Yellow}https://www.openssl.org/source/ ${Font}${Blue}下载您需要升级的OpenSSL版本包，放在脚本同目录下。(注意：只能是${Font}${Yellow}openssl-1.x.xx${Font}${Blue}的版本包)\nPlease download the OpenSSL version you need to upgrade from the ${Font}${Yellow}https://www.openssl.org/source/${Font}${Blue},place it in the same directory as the script.\nAttention:It can only be a version package of ${Font}${Yellow}1.x.xx${Font}\n"
			exit 1
...
function dependent_install() {
	...
	judge "安装依赖：gcc gcc-c++ glibc make autoconf openssl openssl-devel pcre-devel\nInstall gcc gcc-c++ glibc make autoconf openssl openssl-devel pcre-devel"
}
...
function telnet_install() {
	...
	judge "安装telnet相关组件：xinetd telnet-server telnet\nInstall telnet: xinetd telnet-server telnet"
	...
	if [[ ${telnet_config} != "4" ]];then
		print_error "Telnet权限配置失败\nTelnet permission configuration failed."
		exit 1
	fi
	...
	if [[ ${telnet_port} == "23" ]];then
		print_ok "Telnet启动成功\nTelnet started successfully."
	else
		print_error "Telnet启动失败\nTelnet startup failed."
		exit 1
	fi
}
...
function openssl_update() {
	...
	judge "解压OpenSSL版本包\nUnzip OpenSSL package."
	cd ${openssl_version}

	# 备份旧版本openssl
	ls -all /usr/bin/openssl
	mv /usr/bin/openssl /usr/bin/openssl_bak_${date_time}
	judge "备份/usr/bin/openssl到/usr/bin/openssl_bak_${date_time}\nBackup /usr/bin/openssl to /usr/bin/openssl_bak_${date_time}"
	ls -all /usr/include/openssl
	mv /usr/include/openssl /usr/include/openssl_bak_${date_time}
	judge "备份/usr/include/openssl到/usr/include/openssl_bak_${date_time}\nBackup /usr/include/openssl to /usr/include/openssl_bak_${date_time}"

	# 编译安装openssl
	cd ${path_way}/${openssl_version} && ./config --prefix=/usr/local/ssl --openssldir=/usr/local/ssl shared && make && make install
	judge "编译安装OpenSSL\nmake && make install OpenSSL"

	# 制作软链接
	ln -s /usr/local/ssl/bin/openssl /usr/bin/openssl
	judge "生成软连接：/usr/bin/openssl -> /usr/local/ssl/bin/openssl\nMaking soft link：/usr/bin/openssl -> /usr/local/ssl/bin/openssl"
	ln -s /usr/local/ssl/include/openssl /usr/include/openssl
	judge "生成软连接：/usr/include/openssl -> /usr/local/ssl/include/openssl\nMaking soft link：/usr/include/openssl -> /usr/local/ssl/include/openssl"
	ls -all /usr/bin/openssl
	...
	judge "加载新配置1：sed -i '/\/usr\/local\/ssl\/lib/d' /etc/ld.so.conf\nLoad new config 1:sed -i '/\/usr\/local\/ssl\/lib/d' /etc/ld.so.conf"
	echo "/usr/local/ssl/lib" >> /etc/ld.so.conf
	judge "加载新配置2：echo \"/usr/local/ssl/lib\" >> /etc/ld.so.conf\nLoad new config 2:echo \"/usr/local/ssl/lib\" >> /etc/ld.so.conf"
	ln -s /usr/local/ssl/lib/libssl.so.1.1 /usr/lib64/libssl.so.1.1
	judge "加载新配置3：ln -s /usr/local/ssl/lib/libssl.so.1.1 /usr/lib64/libssl.so.1.1\nLoad new config 3:ln -s /usr/local/ssl/lib/libssl.so.1.1 /usr/lib64/libssl.so.1.1"
	ln -s /usr/local/ssl/lib/libcrypto.so.1.1 /usr/lib64/libcrypto.so.1.1
	judge "加载新配置4：ln -s /usr/local/ssl/lib/libcrypto.so.1.1 /usr/lib64/libcrypto.so.1.1\nLoad new config 4:ln -s /usr/local/ssl/lib/libcrypto.so.1.1 /usr/lib64/libcrypto.so.1.1"
	export PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin
	judge "加载新配置5：export PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin\nLoad new config 5:export PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin"
	/sbin/ldconfig
	judge "加载新配置6：/sbin/ldconfig\nLoad new config 6:/sbin/ldconfig"
	ldconfig
	judge "加载新配置7：ldconfig\nLoad new config 7:ldconfig"
...
function openssh_update() {
	...
	judge "解压OpenSSH版本包\nUnzip OpenSSH package."
	cd ${openssh_version}
	echo ${openssh_version} > ${path_way}/.ssh_version.tmp

	# 备份openssh
	cp -rp /etc/ssh/ /etc/ssh_bak_${date_time}/
	judge "备份/etc/ssh/到/etc/ssh_bak_${date_time}/\nBackup /etc/ssh/ to /etc/ssh_bak_${date_time}/"
...
function openssh_restart() {
	...
	judge "加载新配置1：cp -a contrib/redhat/sshd.init /etc/init.d/sshd\nLoad new config 1:cp -a contrib/redhat/sshd.init /etc/init.d/sshd"
	cp -a contrib/redhat/sshd.pam /etc/pam.d/sshd.pam
	judge "加载新配置2：cp -a contrib/redhat/sshd.pam /etc/pam.d/sshd.pam\nLoad new config 2:cp -a contrib/redhat/sshd.pam /etc/pam.d/sshd.pam"
	chmod +x /etc/init.d/sshd
	judge "加载新配置3：chmod +x /etc/init.d/sshd\nLoad new config 3:chmod +x /etc/init.d/sshd"
	...
	judge "设置ssh开启自启\nSet SSH startup and self start."
...
2、After confirming that the function is normal, please use this command to cleanup after upgrade, but does not include backup files: ${Font}${Red}bash ssh_update_onekey.sh clear${Font}"
		...
		;;
	*)
		print_error "升级已停止，请进行相关检查\nUpgrade stopped, please perform relevant checks."
		exit 1
		;;
	esac
}
...
${Blue}请输入您的选择(Please enter your selection)：${Font}"
	...
	1)
		...
		judge "卸载telnet相关组件：xinetd telnet-server\nRemove telnet: xinetd telnet-server"
		;;
	2)
		cp /etc/securetty_bak /etc/securetty_bak${date_time}
		mv /etc/securetty_bak /etc/securetty
		...
		judge "关闭telnet相关组件：xinetd telnet-server\nShutdown telnet: xinetd telnet-server"
		;;
	3)
		cp /etc/securetty_bak /etc/securetty_bak${date_time}
		mv /etc/securetty_bak /etc/securetty
		systemctl restart telnet.socket
		systemctl restart xinetd
		judge "重启telnet相关组件：xinetd telnet-server\nShutdown telnet: xinetd telnet-server"
		;;
	4)
		exit 1
		;;
